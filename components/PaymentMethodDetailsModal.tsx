...
const PaymentMethodDetailsModal: React.FC<PaymentMethodDetailsModalProps> = ({ paymentMethod, onClose }) => {
    const { t } = useContext(LanguageContext);

    if (!paymentMethod) return null;
    const isOpen = !!paymentMethod;
        ...
        <div className={`fixed inset-0 z-50 flex items-start justify-center p-4 transition-opacity duration-300 overflow-y-auto ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} role="dialog">
            ...
            <div className={`relative w-full max-w-4xl my-8 bg-white dark:bg-slate-800 rounded-lg shadow-2xl flex flex-col transform transition-all duration-300 max-h-[90vh] ${isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
                <header className="flex items-center justify-between p-4 border-b dark:border-slate-700 flex-shrink-0 sticky top-0 bg-white dark:bg-slate-800 rounded-t-lg z-10">
                    <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">{t('paymentMethodsPage.detailsTitle')} - {paymentMethod.name_ar}</h2>
                ...
                <div className="flex-grow p-6 overflow-y-auto">
                    <Section title={t('paymentMethodsPage.paymentMethodInfo')}>
                        <DetailItem label={t('paymentMethodsPage.th_id')} value={`#${paymentMethod.id}`} />
                        <DetailItem label={t('paymentMethodsPage.th_name_ar')} value={paymentMethod.name_ar} />
                        <DetailItem label={t('paymentMethodsPage.th_name_en')} value={paymentMethod.name_en} />
                        <DetailItem label={t('paymentMethodsPage.th_status')} value={
                            <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${paymentMethod.status === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300'}`}>
                                {t(`paymentMethodsPage.status_${paymentMethod.status}`)}
                            </span>
                        } />
                    </Section>
                    <Section title={t('bookings.details.timestamps')}>
                        <DetailItem label={t('paymentMethodsPage.th_createdAt')} value={formatDate(paymentMethod.created_at)} />
                        <DetailItem label={t('paymentMethodsPage.th_updatedAt')} value={formatDate(paymentMethod.updated_at)} />
                    </Section>
                </div>
            </div>
        </div>
