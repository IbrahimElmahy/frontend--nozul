...
const reportsData = [
    { id: 'cash_flow', category: 'financial' },
    { id: 'account_statement', category: 'financial' },
    { id: 'balady', category: 'operational' },
    { id: 'daily_bookings', category: 'operational' },
];
                                ...
                                <div key={category.id}>
                                    ...
                                    <div className="space-y-2">
                                        {reportsInCategory.map(report => (
                                            <div key={report.id} className="flex justify-between items-center p-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-lg transition-colors duration-150">
                                                <div>
                                                    <h4 className="font-semibold text-blue-600 dark:text-blue-400">{t(`reportsPage.reports.${report.id}.title` as any)}</h4>
                                                    <p className="text-sm text-slate-500 dark:text-slate-400">{t(`reportsPage.reports.${report.id}.description` as any)}</p>
                                                </div>
                                                <button onClick={() => handleToggleFavorite(report.id)} aria-label="Toggle favorite">
