...
interface AddExpensePanelProps {
    initialData: Omit<Expense, 'id' | 'created_at' | 'updated_at'>;
    isEditing: boolean;
    isOpen: boolean;
    onClose: () => void;
...
const AddExpensePanel: React.FC<AddExpensePanelProps> = ({ initialData, isEditing, isOpen, onClose, onSave }) => {
        ...
        <div className={`fixed inset-0 z-50 flex items-start justify-end transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} role="dialog">
            ...
            <div className={`relative h-full bg-white dark:bg-slate-800 shadow-2xl flex flex-col w-full max-w-lg transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <header className="flex items-center justify-between p-4 border-b dark:border-slate-700 flex-shrink-0 sticky top-0 bg-white dark:bg-slate-800 z-10">
                    <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">{isEditing ? t('expensesPage.editTitle') : t('expensesPage.addTitle')}</h2>
                ...
                <div className="flex-grow p-6 overflow-y-auto">
                    <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
                        <div>
                            <label htmlFor="name_en" className={labelBaseClass}>{t('expensesPage.th_name_en')}</label>
                            <input type="text" name="name_en" id="name_en" value={formData.name_en} onChange={handleInputChange} className={inputBaseClass} />
                        </div>
                        <div>
                            <label htmlFor="name_ar" className={labelBaseClass}>{t('expensesPage.th_name_ar')}</label>
                            <input type="text" name="name_ar" id="name_ar" value={formData.name_ar} onChange={handleInputChange} className={inputBaseClass} />
                        </div>
                        <div>
                            <label htmlFor="description" className={labelBaseClass}>{t('expensesPage.description')}</label>
                        ...
                        <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                            <label htmlFor="status" className="font-semibold text-slate-800 dark:text-slate-200">{t('expensesPage.th_status')}</label>
