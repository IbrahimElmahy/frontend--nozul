            ...
            <div className={`relative w-full max-w-4xl my-8 bg-white dark:bg-slate-800 rounded-lg shadow-2xl flex flex-col transform transition-all duration-300 max-h-[90vh] ${isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
                ...
                <div className="flex-grow p-6 overflow-y-auto">
                    <Section title={t('bookings.bookingInfo')}>
                        {/* FIX: Use correct property 'number' and 'reservation' */}
                        <DetailItem label={t('receipts.th_invoiceNumber')} value={invoice.number} />
                        <DetailItem label={t('receipts.th_bookingNumber')} value={invoice.reservation} />
                    </Section>
                    
                    <Section title={t('bookings.financialInfo')}>
                        {/* FIX: Use correct property 'amount' and handle optional properties */}
                        <DetailItem label={t('receipts.th_value')} value={invoice.amount.toFixed(2)} />
                     ...
                     <Section title={t('bookings.details.timestamps')}>
                        {/* FIX: Use correct properties 'created_at' and 'updated_at' */}
                        <DetailItem label={t('receipts.th_createdAt')} value={formatDate(invoice.created_at)} />
                        <DetailItem label={t('receipts.th_updatedAt')} value={formatDate(invoice.updated_at)} />
                    </Section>
                </div>
            </div>
        </div>
