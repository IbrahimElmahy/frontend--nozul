...
const IntegrationRequestModal: React.FC<IntegrationRequestModalProps> = ({
    ...
    const handleDateChange = (date: string) => {
        setFormData(prev => ({ ...prev, activationDate: date }));
        if (errors.activationDate) {
            setErrors(prev => ({ ...prev, activationDate: '' }));
        }
    };
    ...
    const validateForm = (): boolean => {
        ...
        if (formData.systemType === tr.systems.wafiq) {
            ...
            if (!formData.activatedBy.trim()) newErrors.activatedBy = tr.requiredField;
            if (!formData.activationDate.trim()) newErrors.activationDate = tr.requiredField;
        }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (formData.systemType === tr.systems.wafiq) {
            dynamicFieldsBody = language === 'ar' ? `
بيانات نظام وافق:
مفتاح الربط: ${formData.apiKey}` : `
Wafiq System Details:
...
Shomoos System Details:
Username: ${formData.username}
Branch Code: ${formData.branchCode}
Branch Password: ${formData.branchPassword}`;
        } else if (formData.systemType === tr.systems.ntm) {
            dynamicFieldsBody = language === 'ar' ? `
بيانات نظام الرصد السياحي:
مفتاح الربط: ${formData.connectionKey}
اسم المستخدم: ${formData.username}
كلمة المرور: ${formData.password}
تم التفعيل بواسطة: ${formData.activatedBy}
تاريخ التفعيل: ${formData.activationDate}` : `
Tourism Monitoring System Details:
Connection Key: ${formData.connectionKey}
Username: ${formData.username}
Password: ${formData.password}
Activated By: ${formData.activatedBy}
Activation Date: ${formData.activationDate}`;
        }
...
---
تم إرسال هذا الطلب من نظام نزلكم`
...
---
This request was sent from Nozulkom System`
        );

        const mailtoLink = `mailto:support@nozulkom.com?subject=${emailSubject}&body=${emailBody}`;
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    {formData.systemType === tr.systems.ntm && (
                        <div className="space-y-4 bg-slate-50 dark:bg-slate-700/30 p-4 rounded-lg border border-slate-200 dark:border-slate-600">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className={labelClass}>{tr.dynamicFields.activatedBy} *</label>
                                    <input
                                        type="text"
                                        name="activatedBy"
                                        value={formData.activatedBy}
                                        onChange={handleInputChange}
                                        className={`${inputClass} ${errors.activatedBy ? 'border-red-500' : ''}`}
                                    />
                                    {errors.activatedBy && <p className={errorClass}>{errors.activatedBy}</p>}
                                </div>
                                <div>
                                    <label className={labelClass}>{tr.dynamicFields.activationDate} *</label>
                                    <DatePicker value={formData.activationDate} onChange={handleDateChange} />
                                    {errors.activationDate && <p className={errorClass}>{errors.activationDate}</p>}
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <div className={`flex gap-3 pt-4 ${language === 'ar' ? 'flex-row-reverse' : ''}`}>
                        <button
                            type="submit"
